...
async function approve(req, res) {
  try {
    await AdminCompanyApprovalService.approve(parseInt(req.params.id, 10), req.session.userId, req.ip);
...
async function reject(req, res) {
  try {
    await AdminCompanyApprovalService.reject(parseInt(req.params.id, 10), req.body.reason, req.session.userId, req.ip);
    res.redirect(backTo(req, '/admin/approvals'));
  } catch (err) {
